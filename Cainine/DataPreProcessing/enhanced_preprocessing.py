"""
Enhanced ECG Preprocessing for Canine Transfer Learning
...
def main():
    """Process canine ECG data from Combined_data folder"""
    ...
    print("="*60)
    print("CANINE ECG ENHANCED PREPROCESSING")
    print("="*60)
    ...
    for signal_file in signal_files:
        ...
        file_config = PreprocessConfig(
            ...
            output_file=os.path.join(output_dir, f"{base_name}_beats_enhanced.csv"),
            window_len=base_config.window_len,
            sampling_rate=base_config.sampling_rate,
            use_global_normalization=base_config.use_global_normalization,
    ...
    if total_written > 0:
        print("\n[combining] Creating combined dataset...")
        combined_file = os.path.join(output_dir, "canine_all_enhanced.csv")
        try:
            combine_batch_files(
                os.path.join(output_dir, "*_beats_enhanced.csv"),
                combined_file,
                base_config
            )
            print(f"[done] Combined dataset saved to: {combined_file}")
        except Exception as e:
